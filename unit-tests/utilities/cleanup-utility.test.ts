...
test.describe("CleanupRegistry", () => {
  ...
  test.describe("registerEntity()", () => {
    ...
    test("should work with different entity types (generics)", async () => {
      ...
      const resultString = registry.registerEntity(
        stringEntity,
        async () => {},
      );
      const resultNumber = registry.registerEntity(
        numberEntity,
        async () => {},
      );
      const resultObject = registry.registerEntity(
        objectEntity,
        async () => {},
      );
  ...
  test.describe("executeAll()", () => {
    ...
    test("should allow re-registering after executeAll clears", async () => {
      ...
      registry.register(async () => {
        executionOrder.push("first-batch");
      });
      await registry.executeAll();

      registry.register(async () => {
        executionOrder.push("second-batch");
      });
      await registry.executeAll();
