...
test.describe('DataFactory', () => {
  ...
  test.describe('candidate() - default values', () => {
    ...
    test('should default vacancy to Software Engineer', () => {
      const candidate = dataFactory.candidate();

      expect(candidate.vacancy).toBe('Software Engineer');
    });
    ...
    test('should generate keywords from predefined list', () => {
      const candidate = dataFactory.candidate();
      const validKeywords = [
        'JavaScript', 'TypeScript', 'Python', 'Java',
        'Testing', 'QA', 'Automation', 'React', 'Node.js', 'SQL'
      ];
      const candidateKeywords = candidate.keywords!.split(', ');

      candidateKeywords.forEach(keyword => {
        expect(validKeywords).toContain(keyword);
      });
    });
